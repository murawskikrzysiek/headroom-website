"""
make_ls_lyra.py
Lemon Squeezy store thumbnail for Lyra — 1600×1200.

Design: Dark Instrument
A control-room-at-night aesthetic. Icon as primary object, centred.
F10/F11/F12 shown as sculpted key-cap elements. Minimal type system.
"""
...
FONTS     = "/sessions/intelligent-hopeful-brown/mnt/.skills/skills/canvas-design/canvas-fonts"

W, H = 1600, 1200

# ── Palette ────────────────────────────────────────────────────────────────────
BG      = (11,  11,  15)
ACCENT  = (124, 132, 246)   # purple
TEXT    = (238, 238, 246)
TEXT2   = (110, 110, 144)
TEXT3   = (45,  45,  65)
GREEN   = (62,  207, 124)   # brand green
...
key_w, key_h = 130, 66
gap          = 28
total_keys_w = len(keys) * key_w + (len(keys) - 1) * gap
keys_x0      = (W - total_keys_w) // 2
keys_y       = sep_y + 100

for i, (label, action) in enumerate(keys):
...
# ── Save ──────────────────────────────────────────────────────────────────────
out = os.path.join(HERE, "lyra/ls-thumbnail.png")
img.save(out, "PNG")
