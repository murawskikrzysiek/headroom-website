...
| `guide-toc.js` | sidebar-TOC upgrades for long guides: injects a fuzzy search field (titles fuzzy, body text substring, subhead deep-links) and scroll-spies the current section and subhead; wants `nav.toc[data-guide-toc]`; layout (sticky rail >=1100px, static card below) lives in the page's own CSS. Indexes **direct-child `li` only**, so a nested `.toc-sub` is not mistaken for a section. A page that ships its own static `.toc-sub` owns those nodes: search filters them and never rebuilds them (Videre); a page without one gets sub-entries derived from its h3s while a search runs (everyone else). Shell furniture - not part of the md mirror | all four guides + both API pages (a value on `data-guide-toc` overrides the search placeholder) |
...
- `lyra/guide.html` mirrors `Lyra/USER-GUIDE.md`; `specula/guide.html` mirrors `Specula/USER-GUIDE.md`; `videre/guide.html` mirrors `Videre/USER-GUIDE.md`. **Edit the markdown in the app repo, then re-mirror.** The md sources use em-dashes; the mirror step converts them to spaced hyphens — preserve that. The masthead (h1/lede) is website furniture, not in the md — edit it HTML-side.
- `audita/guide.html` has **no markdown source** — edit the HTML directly.
- **Videre's guide numbers its subheads** (`### 6.1 Gamut` in the md → `<h3 id="scopes-gamut">6.1 · Gamut…` plus an `.anchor` link in the HTML), and the Contents rail lists them as a static `<ul class="toc-sub">` inside the section's `<li>`. Section numbers are display-only — the anchors (`#scopes`, `#conform`) are what everything links to, so renumbering never breaks a link. If you add or reorder a subhead, renumber both files and add the matching TOC row; the ids are full-title slugs prefixed with the section id. Rail labels drop the shortcut parenthetical that the heading keeps (`6.1 · Gamut`, not `6.1 · Gamut (⌃1)`) because a 240px rail wraps otherwise.
